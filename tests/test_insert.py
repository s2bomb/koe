...
def test_insert_transcript_text_returns_ok_when_write_and_paste_succeed() -> None:
    with (
        patch.object(
            koe_insert,
    ...
    write_mock.assert_called_once_with("hello", "hello")
    paste_mock.assert_called_once_with(DEFAULT_CONFIG, "hello")
...
def test_insert_transcript_text_enforces_stage_order() -> None:
    ...
    assert result == {"ok": True, "value": None}
    assert events == ["write_clipboard_text", "simulate_paste"]
...
def test_insert_transcript_text_short_circuits_on_paste_failure() -> None:
    ...
    write_mock.assert_called_once_with(transcript_text, transcript_text)


def test_insert_transcript_text_success_does_not_use_clipboard_restore_symbol() -> None:
    assert not hasattr(koe_insert, "restore_clipboard_text")
...
def test_write_clipboard_text_maps_nonzero_exit_to_prefixed_error() -> None:
    transcript_text = "original transcript"
...
def test_write_clipboard_text_maps_execution_failure_to_prefixed_error() -> None:
    transcript_text = "original transcript"
